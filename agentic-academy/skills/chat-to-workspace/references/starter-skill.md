...
**one** strong starter skill (the highest-frequency, most-systematizable pattern). The full
Skills Library Builder later in the cohort produces the rest; the job here is to ship one
real, runnable skill so the person sees the payoff on day one.
...
bar**, using these signals in order:
...
to justify a starter skill. Pick the one task you could write the clearest "what good looks
like" for; a sharp narrow skill beats a vague broad one.
...
2. Fill every bracket from real evidence:
   - **description** — include the exact phrases the person uses to ask for this, so it
     triggers reliably. Quote from `samples.md` / `memory.md`.
...
4. Keep it to ONE skill. Resist scaffolding a library — that's a later cohort deliverable.
